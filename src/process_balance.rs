...
pub fn process_transactions(file_path: &str) -> io::Result<()> {
    let transactions = read_json(file_path)?;
    let coin_path = format!("{}/data/coin.tsv", project_path::PATH);
    let mut _coin_file = File::create(coin_path)?;

    let indexer_path = format!("{}/data/indexer.tsv", project_path::PATH);
        ...
        if let Some(op_value) = transaction.inscription_body.get("op") {
            ...
            if op_str == "deploy" {
                ...
                (
                    ...
                    if !tick_exists_in_coin_tsv(&tick_str)? {
                        write_to_coin_tsv(&tick_str, &max_str, &limit_str, &transaction.id)?;
                    ...
                    if !tick_exists_in_coin_tsv(&tick_str)? && !inscription_id_exists_in_failinscription_tsv(&transaction.txid)?
                    {
                        write_to_failinscription_tsv(&transaction.txid)?;
                    }
                    // If tick has not been deployed but is already recorded in failinscription, then skip
                    else if !tick_exists_in_coin_tsv(&tick_str)? && inscription_id_exists_in_failinscription_tsv(&transaction.txid)?
                    { continue;}
                    else if tick_exists_in_coin_tsv(&tick_str)? 
                    {
                        
                        let (leftforsupply, limit) = read_coin_tsv(tick_str).expect("Error reading coin.tsv");
                        ...
                        if limit_numeric < amt_numeric && !inscription_id_exists_in_failinscription_tsv(&transaction.txid)?{
                            write_to_failinscription_tsv(&transaction.txid)?;
                        }
                        ...
                        if leftforsupply_numeric >= amt_numeric && limit_numeric >= amt_numeric {
                            update_coin_tsv(tick_str, amt_numeric).expect("Error updating coin.tsv");
                            update_indexer_tsv(tick_str, &transaction.address, amt_numeric, &transaction.id).expect("Error updating indexer.tsv");                            
                        ...
                        if leftforsupply_numeric < amt_numeric && limit_numeric >= amt_numeric {
                            update_coin_tsv(tick_str, leftforsupply_numeric).expect("Error updating coin.tsv when leftforsupply_numeric < amt_numeric");
                            update_indexer_tsv(tick_str, &transaction.address, leftforsupply_numeric, &transaction.id).expect("Error updating indexer.tsv when leftforsupply_numeric < amt_numeric");
                        }
                    }
                }
            }
                ...
                = 
                (transaction.inscription_body.get("tick"),transaction.inscription_body.get("amt"),)
                {
                    let tick_str = tick_value.as_str().unwrap_or_default();
                    let amt_str = amt_value.as_str().unwrap_or_default();
                    // If transaction.txid exists in failinscription.tsv, then skip
                    if inscription_id_exists_in_failinscription_tsv(&transaction.txid)? {
                        continue;
                    }
                    // If transaction.txid does not exist in failinscription.tsv
                    if !inscription_id_exists_in_failinscription_tsv(&transaction.txid)? {
                        // Check if tick\address exists in indexer.tsv
                        let _ = process_indexer_entry(tick_str,&transaction.address,amt_str,&transaction.txid,&transaction.id);
                    }
                }
            }
            else{continue;}
...
fn process_indexer_entry(
    ...
    let indexer1_path = format!("{}/data/indexer.tsv", project_path::PATH);
    let temp_path = format!("{}/data/indexer_temp.tsv", project_path::PATH);
    ...
    for line_result in reader.lines() {
        ...
        if let (Some(existing_tick), Some(existing_address), Some(availablebalance), Some(transferablebalance)) =
            (columns.get(0), columns.get(1), columns.get(3), columns.get(4))
        {
            if existing_tick == &tick_str && existing_address == &address {
                ...
                if availablebalance_numeric < amt_numeric {
                    ...
                    write_to_failinscription_tsv(&txid)?;
                } else {
                   
                    let updated_availablebalance = availablebalance_numeric - amt_numeric;
                    let updated_transferablebalance = transferablebalance.parse::<i32>().unwrap_or_default() + amt_numeric;
    ...
    let indexer2_path = format!("{}/data/indexer.tsv", project_path::PATH);
    
    let temp2_path = format!("{}/data/indexer_temp.tsv", project_path::PATH);
...
fn update_indexer_tsv(tick_str: &str, address: &str, amt_numeric: i32, id: &str) -> io::Result<()> {
    let indexer2_path = format!("{}/data/indexer.tsv", project_path::PATH);
    let temp2_path = format!("{}/data/indexer_temp.tsv", project_path::PATH);

    let indexer_file = File::open(indexer2_path)?;
    let mut temp_file = OpenOptions::new().create(true).write(true).open(temp2_path)?;
    ...
    let indexer3_path = format!("{}/data/indexer.tsv", project_path::PATH);
    let temp3_path = format!("{}/data/indexer_temp.tsv", project_path::PATH);
...
fn update_coin_tsv(tick_str: &str, amt_numeric: i32) -> io::Result<()> {
    
    let coin5_path = format!("{}/data/coin.tsv", project_path::PATH);
    let temp5_path = format!("{}/data/coin_temp.tsv", project_path::PATH);

    let coin_file = File::open(coin5_path)?;
    let mut temp_file = OpenOptions::new().create(true).write(true).open(temp5_path)?;
    ...
    let coin6_path = format!("{}/data/coin.tsv", project_path::PATH);
    let temp6_path = format!("{}/data/coin_temp.tsv", project_path::PATH);
...
fn read_coin_tsv(name: &str) -> io::Result<(String, String)> {
    let coin6_path = format!("{}/data/coin.tsv", project_path::PATH);
...
fn tick_exists_in_coin_tsv(tick: &str) -> io::Result<bool> {
    let coin7_path = format!("{}/data/coin.tsv", project_path::PATH);
...
fn inscription_id_exists_in_failinscription_tsv(txid: &str) -> io::Result<bool> {
    let failinscription_path = format!("{}/data/failinscription.tsv", project_path::PATH);
...
fn write_to_coin_tsv(tick: &str, leftforsupply: &str, limit: &str, id: &str) -> io::Result<()> {
    let coin8_path = format!("{}/data/coin.tsv", project_path::PATH);
    let mut coin_file = OpenOptions::new().create(true).append(true).open(coin8_path)?;
    writeln!(coin_file, "{}\t{}\t{}\t{}", tick, leftforsupply, limit, id)?;
    Ok(())
}

fn write_to_failinscription_tsv(txid: &str) -> io::Result<()> {
    let failinscription1_path = format!("{}/data/failinscription.tsv", project_path::PATH);
    let mut failinscription_file = OpenOptions::new().create(true).append(true).open(failinscription1_path)?;
    writeln!(failinscription_file, "{}i0", txid)?;
    Ok(())
}

fn read_json(file_path: &str) -> io::Result<Vec<Transaction>> {
