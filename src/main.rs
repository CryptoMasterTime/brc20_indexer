mod get_ordinals_export_data;
mod export_data_to_json;
mod inscriptions;
mod process_balance;
mod config;
...
fn main() {
    ...
    let export_data_path = format!("{}/data/exportdata", project_path::PATH);
    get_ordinals_export_data::export_index_data(&export_data_path);
    let export_json = format!("{}/data/id_txid_addr.json", project_path::PATH);
    if let Err(err) = export_data_to_json::read_export_data(&export_data_path, &export_json) {
        eprintln!("Error reading/exporting data: {}", err);
    }
    let _ = inscriptions::process_transactions();
...
mod tests {
    ...
    fn test_check_chaindata_parse() {
        let ord_path = String::from("/Users/CryptoMaster...ordinals/ord/target/release/ord"); 
        let ord_cookie = String::from("/Users/CryptoMaster.../ordinals/ord/target/release/.cookie");
        let delandlabsindexer_path = String::from("/Users/CryptoMaster...");
        ...
        let transfer_str = format!("{}tests/testdata/brc20_transfer.txt",&delandlabsindexer_path);
        let indexerbin_str = format!("{}target/debug/delandlabsindexer",&delandlabsindexer_path);
        let _jsonscript_str = format!("{}src/data/id_txid_inscription_addr.json",delandlabsindexer_path);
        ...
        if cmd1_output.status.success() {
            ...
            eprintln!("cmd1 failed with exit code: {:?}", cmd1_output.status.code());
        ...
        if cmd2_output.status.success() {
            ...
            eprintln!("cmd1 failed with exit code: {:?}", cmd2_output.status.code());
        ...
        if cmd3_output.status.success() {
            ...
            eprintln!("cmd3 failed with exit code: {:?}", cmd3_output.status.code());
        ...
        if cmd4_output.status.success() {
            ...
            eprintln!("cmd4 failed with exit code: {:?}", cmd4_output.status.code());
        ...
        if cmd5_output.status.success() {
            ...
            eprintln!("cmd5 failed with exit code: {:?}", cmd5_output.status.code());
        ...
        let transactions: Vec<Transaction> = serde_json::from_value(json_value).expect("Failed to parse Transaction");
            ...
            if let Some(op_value) = transaction.inscription_body.get("op") {
                ...
                if op_str == "deploy" {
                    if let Ok(brc20_deploy) = serde_json::from_str::<Brc20Deploy>(&deploy_json_str) {
                        ...
                        if let Some(inscription_p) = transaction.inscription_body.get("p") {
                            ...
                            assert_eq!(inscription_p_str, p_str, "Comparison for p field in deploy operation failed");
                        } else {
                        ...
                        if let Some(inscription_max) = transaction.inscription_body.get("max") {
                            ...
                            assert_eq!(inscription_max_str, max_str, "Comparison for max field in deploy operation failed");
                        } else {
                        ...
                        if let Some(inscription_lim) = transaction.inscription_body.get("lim") {
                            ...
                            assert_eq!(inscription_lim_str, lim_str, "Comparison for lim field in deploy operation failed");
                        } else {
                        ...
                        if let Some(inscription_tick) = transaction.inscription_body.get("tick") {
                            let inscription_tick_str = inscription_tick.as_str().unwrap_or_default();
                            assert_eq!(inscription_tick_str, tick_str, "Comparison for tick field failed");
                        } else {
                        ...
                        if let Some(inscription_op) = transaction.inscription_body.get("op") {
                            ...
                            assert_eq!(inscription_op_str, op_str, "Comparison for op field failed");
                        } else {
                ...
                if op_str == "mint" {
                    if let Ok(brc20_mint) = serde_json::from_str::<Brc20Operation>(&mint_json_str) {
                        ...
                        if let Some(inscription_p) = transaction.inscription_body.get("p") {
                            let inscription_p_str = inscription_p.as_str().unwrap_or_default();
                            assert_eq!(inscription_p_str, p_str, "Comparison for p field in mint operation failed");
                        } else {
                        ...
                        if let Some(inscription_amt) = transaction.inscription_body.get("amt") {
                            ...
                            assert_eq!(inscription_amt_str, amt_str, "Comparison for amt field in mint operation failed");
                        } else {
                ...
                if op_str == "transfer" {
                    if let Ok(brc20_transfer) = serde_json::from_str::<Brc20Operation>(&transfer_json_str) {
                        // Processing for brc20_transfer, adapt based on your actual scenario
                        let p_str = brc20_transfer.p;
                        if let Some(inscription_p) = transaction.inscription_body.get("p") {
                            let inscription_p_str = inscription_p.as_str().unwrap_or_default();
                            assert_eq!(inscription_p_str, p_str, "Comparison for p field in transfer operation failed");
                        } else {
                        ...
                        if let Some(inscription_amt) = transaction.inscription_body.get("amt") {
                            let inscription_amt_str = inscription_amt.as_str().unwrap_or_default();
                            assert_eq!(inscription_amt_str, amt_str, "Comparison for amt field in transfer operation failed");
                        } else {
    ...
    fn test_brc20_jsonscript_analyse() {
        use crate::config::project_path;
        let export_json = format!("/Users/CryptoMaster.../tests/testdata/id_txid_inscription_addr_test.json");
        let _ = crate::process_balance::process_transactions(&export_json);
        let export_indexer = format!("{}/data/indexer.tsv", project_path::PATH);
